...
@section('title', 'Semua Dokumen - Bahaudin Digital Repository')
@section('content')
        ...
        <div class="flex flex-col md:flex-row gap-8">
            ...
            <aside class="w-full md:w-64 flex-shrink-0">
                <div class="bg-white p-6 rounded-lg shadow-sm sticky top-4">
                    <h3 class="font-bold text-lg mb-4 text-gray-800">Filter</h3>

                    <form action="{{ route('all.documents') }}" method="GET">
                        <!-- Input Pencarian -->
                        <div class="mb-6">
                        ...
                        @if(request()->hasAny(['q', 'year', 'faculty_id', 'document_type_id']))
            ...
            <div class="flex-grow">
                <div class="bg-white p-6 rounded-lg shadow-sm mb-6">
                    <h2 class="text-2xl font-bold text-gray-800 mb-6">
                        Semua Dokumen
                        <span class="text-lg font-normal text-gray-600">({{ $documents->total() }} ditemukan)</span>
                    </h2>
                                ...
                                <div class="flex-grow">
                                    ...
                                    <p class="text-gray-600 mb-2">
                                        ...
                                        @foreach($document->authors as $author)
                                            <span class="text-green-600">{{ $author->name }}{{ !$loop->last ? ',' : '' }}</span>
                                        @endforeach
                                        ({{ $document->publication_year }})
                                    </p>
                                    <p class="text-gray-700 mb-3">
                                        {{ Str::limit($document->abstract_id, 200) }}
                                    </p>
                                    <div class="mt-2 flex flex-wrap items-center text-sm text-gray-500">
                                        ...
                                        <span class="mr-4"><i class="fas fa-university mr-1"></i> {{ $document->faculty->name }}</span>
                                        <span><i class="fas fa-tag mr-1"></i> {{ $document->documentType->name }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    @empty
                        <p class="text-gray-600">Belum ada dokumen ditemukan.</p>
                    @endforelse
